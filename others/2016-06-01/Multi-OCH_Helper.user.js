...
// @include     /^http:\/\/www\.tusfiles\.net\/\w+$/
...
// @include     /^http:\/\/www\d*\.zippyshare\.com\/v\/\d+\/file\.html$/
...
const s_myname = "Multi-OCH Helper";
const h_myname = "Multi-OCH Helper Highlight links";
...
var settings = {};
// Load settings
var savedsettings = JSON.parse(GM_getValue("settings","{}")); // e.g. {  position : [0,1], newTab : 1  }
...
var show_oneclick_FromHighlighScript_allLinks = false;
...
var multi = {
  'premiumize.me' : new (function() {
    ...
    this.getLinks = function(urls,cb) {
      showConfirm("fairPointsWarning","You will be charged premiumize fair points for generating "+(urls.length>1?("<b>"+urls.length+"</b> files"):("<b>one</b> file"))+"!<br><br>Generate links?",function() { this._getLinks(urls, cb); },function() { setStatus("Operation canceled!",0); cb([],-1); },self);
    };
    ...
    this._getSingleLink = function(url, cb) {
      GM_xmlhttpRequest({
        ...
        headers: {
          ...
          "Referer" : "https://www.premiumize.me/downloader"
        },
        onerror: function(response) {
          console.log("GM_xmlhttpRequest error: "+self.homepage+"api/transfer/create");
  ...
  'nopremium.pl' : new (function() {
    ...
    this.loadSettings = function() {
      ...
      var savedsettings = JSON.parse(GM_getValue(this.key+"_settings","{}"));
      ...
      for(var key in this.config) {
        ...
        if(key in savedsettings) { // Saved
          if(this.config[key] == 'int') { // Int
            this.settings[key] = parseInt(savedsettings[key],10);
          }else if(config[key] == 'bool') { // Bool
            this.settings[key] = savedsettings[key]=="true"||savedsettings[key]===true;
          } else if(Array.isArray(savedsettings[key])) { // Nested array
            this.settings[key] = [];
            for(var i = 0; i < savedsettings[key].length; i++) {
              this.settings[key].push(savedsettings[key][i]);
            }
          } else { // Array
          ...
          if(this.config[key] == 'int') { // Int
            this.settings[key] = this.config[key+"_range"][1];
        } else if(config[key] == 'bool') { // Bool
            this.settings[key] = true;
          } else if(Array.isArray(this.config[key][0])) { // Nested array defaults to first value for each array
            this.settings[key] = [];
            for(var i = 0; i < this.config[key].length; i++) {
              this.settings[key].push(this.config[key][i][0]);
            }
          } else {
            this.settings[key] = this.config[key][0]; // Array defaults to first value
          }
        }
      }
    };
    ...
    var getHashs = function(urls,cb,silent) {
      ...
      GM_xmlhttpRequest({
        ...
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
          "Cache-Control" : "no-cache",
          "Referer" : "https://www.nopremium.pl/files"
        },
        onload: function(response) {
    ...
    this.getResults = function(urls,cb,hashes) {
      ...
      if(typeof hashes == "undefined") {
        ...
        getHashs(urls,function(hashes,size) {
          if(settings.mode == 'transfer') {
            showConfirm("transferWarning","You will be charged <b>"+size+"</b> 'Transfer' for generating "+(hashes.length>1?("<b>"+hashes.length+"</b> files"):("<b>one</b> file"))+"!<br><br>Generate links?",function() { this.getResults(urls,cb,hashes); },null,self);
      ...
      this.getResults(urls,function($node,N) {
        ...
        */
        ...
        for(var i = 0; i < N; i++) {
          ...
          cacheLink([urls[i]],files[i][0],[files[i][1]],self.key); // CACHE single URLs
        }

        cacheLink(urls,new Date(),result,self.key); // CACHE all URLs
        ...
        onload: function(response) {
          ...
          */
          ...
          if(result.length === N) {
            ...
            for(var i = 0;  i < N; i++) {
              if(progess[i]) {
                h += '<span style="display:block; width:'+Math.ceil(progess[i]*1.2)+'px; height:1px; background:white; border-top:'+dotheight+'px dotted green; margin-bottom:1px;"></span>';
              } else {
...
var currentdebrid = GM_getValue("currentdebrid",debridprovider[0]);

for(var key in multi) {
  if(multi[key] == currentdebrid) continue;
  GM_registerMenuCommand(s_myname+" - Switch to "+multi[key].name, (function(key) { return function() {
    if(!confirm(s_myname+"\n\nSet multi-download provider:\n"+multi[key].name)) return;

    GM_setValue("currentdebrid",key);
    currentdebrid = key;
    document.location.reload();
  }})(key)
  );
}
GM_registerMenuCommand(s_myname+" - Delete cached links", function() {
  if(!confirm(s_myname+"\n\nReally delete cached links?")) return;

  GM_setValue("cachedDownloadLinks","{}")

  alert(s_myname+"\n\nCache is empty!");
});
GM_registerMenuCommand(s_myname+" - Restore dialogs and warnings", function() {
...
function saveSettings(ev) {
  ...
  GM_setValue("currentdebrid",currentdebrid);
  GM_setValue("settings",JSON.stringify(newsettings.general));
  for(var key in multi) {
    GM_setValue(key+"_settings",JSON.stringify(newsettings[key]));
  }
...
function aboutMe() {
  ...
  $frame.bind('load', function(e) {
      ...
      $('<input type="button">').val("Clear cache ("+ humanBytes(GM_getValue("cachedDownloadLinks","{}").length-2)+")").appendTo($form).click(function() {
        if(!confirm(s_myname+"\n\nReally delete cached links?")) return;

        GM_setValue("cachedDownloadLinks","{}");

        this.value = "Clear cache ("+ humanBytes(GM_getValue("cachedDownloadLinks","{}").length-2)+")";
        alert(s_myname+"\n\nCache is empty!");
      });
      ...
      $('<input type="button">').val("Restore dialogs and warnings").appendTo($form).click(function() {
        if(!confirm(s_myname+"\n\nReally restore all dialogs and warnings?")) return;
...
function inspectGMvalues() {
  ...
  $frame.bind('load', function(e) {
      ...
      if(keys.length && typeof keys[0] == "undefined") { // Firefox 35+ workaround
        keys = cloneInto(GM_listValues(), window);
      }
...
function GM_xmlhttpRequestBinary(obj) {
...
function showConfirm(id,text,cb_yes,cb_no,thisArg) {
  // Skip
  var dialogs = JSON.parse(GM_getValue("dialogs","[]"));
  ...
  var $remember = $('<input type="checkbox" value="remember">').click(function() {
    if(this.checked) {
      var dialogs = JSON.parse(GM_getValue("dialogs","[]"));
      if(dialogs.indexOf(id) === -1) {
        dialogs.push(id);
        GM_setValue("dialogs",JSON.stringify(dialogs));
      }
    } else {
      var dialogs = JSON.parse(GM_getValue("dialogs","[]"));
      if(dialogs.indexOf(id) !== -1) {
...
function useCache(urls,cb) {
  urls = ""+urls;
  var cachedDownloadLinks = JSON.parse(GM_getValue("cachedDownloadLinks","{}")); // [datestring,downloadlink,multihoster]
...
function cacheLink(urls,datetime,downloadLinks,multihoster) {
  ...
  urls = ""+urls;
  var cachedDownloadLinks = JSON.parse(GM_getValue("cachedDownloadLinks","{}"));
  cachedDownloadLinks[urls] = [datetime,downloadLinks,multihoster];
  GM_setValue("cachedDownloadLinks",JSON.stringify(cachedDownloadLinks));
}

function generateLinks(urls,cb) {
  // Check cache
  if(useCache(urls,cb)) {
    return;
  }

  multi[currentdebrid].getLinks(urls,cb);
}
...
function menu(links) {
  ...
  $select.bind("change",function(ev) {
    ...
    if(!$this.parent().find("#remember").length) {
      var $div = $("<div>");
      var $check = $('<input id="remember" type="checkbox" value="remember" title="Remember selection">').click(function() {
        if(this.checked) {
  ...
  $entry.click(function() { alert(links.join("\n")); });
...
function button(label) {
  ...
  var $loader = $("<span>").appendTo($status);
  $loader.attr("id","multiochhelper_status_loader");
  var $statustext = $("<span>").appendTo($status);
...
if(!updatinghosters && GM_getValue("setup",false)) {
  var now = new Date();
  for(var key in multi) {
...
if(document.location.href.indexOf("nopremium.pl") != -1) {
  ...
  // Serienjunkies
  if(!document.getElementById('CINPUT')) { // if not on captcha page
  ...
  if(document.location.href.indexOf("helper.html") != -1) { // if not on captcha page  
    unsafeWindow.close = function() {}; // Prevent popup from beeing closed
    window.addEventListener('message',function filecryptmessage(event) {
      if (event.data && typeof(event.data) == 'object') {
        ...
        var $b = button("Decrypting...");
        var removeImg = loader();
        getFilecryptcc(event.data, removeImg);
      }
    });
  } else if(document.location.href.indexOf("Container") != -1) { // if not on captcha page
    var $b = button("Please open the Click'n'Load Popup");
    $b.click(function() {
      $("#cnl_btn").click();
    });
  }
